...
bool UGraphView::CheckSameNode_LHCode(UGraphNode* Node)
{
	...
	for (auto& RecordedNode : RealNodes)
	{
		...
		{
			TArray<FString> Path = UFunctionTools_GraphWeaver::GetNodePath(this, RecordedNode);
			/*
...
bool UGraphView::CheckIsNodeRemoved(UGraphNode* Node)
{
	...
	case NAConstructMethod::LHCode_G:
		{
			TArray<FString> Path;
			for (auto& Prefix : Node->LHCode_G_Input.ParentCodes)
				Path.Emplace(Prefix + Node->LHCode_G_Input.SelfId);
			for (int32 i = Path.Num() - 1; i >= 0; i--)//完全相同才认定是同一个节点
			{
				for (auto& Removed : NodeRemoved)
				{
					if (Removed == Path[i])
						Path.RemoveAt(i);
				}
...
FGraphViewDataCarrier UGraphView::GetViewDataCarrier()
{
	...
	Carrier.NodeRemoved = NodeRemoved;
	return Carrier;
}
...
void UGraphView::ResetViewFromDataCarrier(FGraphViewDataCarrier& DataCarrier)
{
	...
	NodeRemoved = DataCarrier.NodeRemoved;
}
...
bool UGraphView::ValidateRankingConsistency_Inner(UGraphView* GraphView, TArray<int32>& IgnoredNodeIndices)
{
	...
	{
		if (IgnoredNodeIndices.Num() > 0 && ForkArrived < IgnoredNodeIndices.Num() && IgnoredNodeIndices[ForkArrived] == Node.IndexInRecorded)
		{
			ForkArrived++;
			continue ;
		}
		for (auto& Parent : Node.Family.Parents)
		{
			if (RealNodes[Parent.IndexInRealNodes].Family.Children.Num() > Parent.Ranking
				&& RealNodes[Parent.IndexInRealNodes].Family.Children[Parent.Ranking].IndexInRealNodes == Node.IndexInRecorded)
				continue ;
			GEngine->AddOnScreenDebugMessage(-1, 10.0f, FColor::Red,
				FString::Printf(TEXT("GraphWeaver:ValidateRankingConsistency::As child in parent ranking is wrong, child index: %d, parent index: %d"), Node.IndexInRecorded, Parent.IndexInRealNodes));
			UE_LOG(LogTemp, Error, TEXT("GraphWeaver:ValidateRankingConsistency::As child in parent ranking is wrong, child index: %d, parent index: %d"), Node.IndexInRecorded, Parent.IndexInRealNodes);
			EMPTY_LOG();
			result = false;
		}
		for (auto& Bro : Node.Family.Brothers)
		{
			if (RealNodes[Bro.IndexInRealNodes].Family.Brothers.Num() > Bro.Ranking
				&& RealNodes[Bro.IndexInRealNodes].Family.Brothers[Bro.Ranking].IndexInRealNodes == Node.IndexInRecorded)
				continue ;
			GEngine->AddOnScreenDebugMessage(-1, 10.0f, FColor::Red,
				FString::Printf(TEXT("GraphWeaver:ValidateRankingConsistency::As bro in bro ranking is wrong, self index: %d, bro index: %d"), Node.IndexInRecorded, Bro.IndexInRealNodes));
			UE_LOG(LogTemp, Error, TEXT("GraphWeaver:ValidateRankingConsistency::As bro in bro ranking is wrong, self index: %d, bro index: %d"), Node.IndexInRecorded, Bro.IndexInRealNodes);
			EMPTY_LOG();
			result = false;
		}
		for (auto& Child : Node.Family.Children)
		{
			if (RealNodes[Child.IndexInRealNodes].Family.Parents.Num() > Child.Ranking
				&& RealNodes[Child.IndexInRealNodes].Family.Parents[Child.Ranking].IndexInRealNodes == Node.IndexInRecorded)
				continue ;

			GEngine->AddOnScreenDebugMessage(-1, 10.0f, FColor::Red,
				FString::Printf(TEXT("GraphWeaver:ValidateRankingConsistency::As parent in child ranking is wrong, parent index: %d, child index: %d"), Node.IndexInRecorded, Child.IndexInRealNodes));
			UE_LOG(LogTemp, Error, TEXT("GraphWeaver:ValidateRankingConsistency::As parent in child ranking is wrong, parent index: %d, child index: %d"), Node.IndexInRecorded, Child.IndexInRealNodes);
			EMPTY_LOG();
			result = false;
		}
	}
...
void UGraphView::AddRemovedNodeName(TArray<FGraphNodeDescription>& Removed)
{
	switch (ConstructMethod)
	{
	case NAConstructMethod::Names:
	...
	case NAConstructMethod::LHCode_G:
		{
			for (auto& Node : Removed)
			{
				TArray<FString> path = UFunctionTools_GraphWeaver::GetNodePath(this, Node);
				for (auto& p : path)
					NodeRemoved.Emplace(p);
			}
			break ;
		}
	default:
...
void UGraphView::AddRemovedNodeName(FGraphNodeDescription& Removed)
{
	switch (ConstructMethod)
	{
	case NAConstructMethod::Names:
		{
			NodeRemoved.Emplace(Removed.NamesInputNodeMirror.SelfName);
			break ;
		}
	case NAConstructMethod::LHCode_G:
		{
			TArray<FString> path = UFunctionTools_GraphWeaver::GetNodePath(this, Removed);
			for (auto& p : path)
				NodeRemoved.Emplace(p);
			break ;
		}
